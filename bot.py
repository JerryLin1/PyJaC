...
a_file.close()
b_file = open("emoji_ids.json", "r")
emoji_ids = json.load(b_file)
...
async def send_weather(ctx, spec_city: str = "", spec_country: str = ""):
    ...
    message = EMOJI_LINE
    message += f":bar_chart: __Here is your weather report! __ <@{user_id}>\n"
...
async def cmd_get_cities(ctx):
    ...
    :param ctx: The context of the Discord message.
    """
    message = f"<@{ctx.author.id}> Here's your saved cities:"
...
def generate_city_data(user_name: str, city: str, country: str) -> str:
    ...
    if country != "":
        message += f", {country}"
    emoji_id = f"{icon}d2x"
    message += f" {temp}C°** {emoji_ids[emoji_id]} \n"
...
def get_special_cond(temp: str, wind_speed: str, humidity: str):
    ...
    if windy:
        warnings.append(
            ':point_right::exclamation:It\'s very windy outside. '
